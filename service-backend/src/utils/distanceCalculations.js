...
export function calculateFare(distanceText) {
  const distanceKm = parseFloat(distanceText.split(' ')[0]);
  return Math.round(distanceKm * 68); // 1km = 68
}

export function calculateRouteProgress(ride) {
  if (!ride.driverLocation || !ride.startLocation || !ride.endLocation) return 0;
